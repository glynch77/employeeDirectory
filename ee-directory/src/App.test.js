...
import EmployeeCard from './components/EmployeeCard'; // Adjust the path to the EmployeeCard component
...
describe('EmployeeCard Component', () => {
  ...
  test('Should display employee details correctly', async () => {
    try {
      await driver.get('http://192.168.1.97:3000'); // Adjust your app's URL
    } catch (err) {
...
describe('EmployeeList Component', () => {
  ...
  test('should render employee details correctly', async () => {
    try {
      await driver.get('http://192.168.1.97:3000'); // Replace with your app's URL
    } catch (err) {
      console.error("Error loading the page:", err);
      throw err; // Re-throw the error so the test fails
    }
    ...
    for (let row of rows) {
      ...
      expect(profilePic).toMatch('https://www.google.com/url?sa=i&url=https%3A%2F%2Fginsura.com%2Femployees%2F&psig=AOvVaw2Rot_F1y5Mxn0MKjdi2CbN&ust=1734929852639000&source=images&cd=vfe&opi=89978449&ved=0CBQQjRxqFwoTCIjOjMHLuooDFQAAAAAdAAAAABAE'); // Check that the src starts with http or https
  ...
  test('should render fallback for missing employee details', async () => {
    ...
    for (let row of rows) {
      ...
      if (!phone) {
        (phone).toBe('N/A'); // or any fallback value you have in place
      }
      if (!email) {
        (email).toBe('N/A'); // or any fallback value you have in place
      }
    }
  });
});
...
describe('Pagination Component', () => {
  ...
  test('Dropdown for items per page is functional', async () => {
    try {
      await driver.get('http://192.168.1.97:3000');
    } catch (err) {
      console.error("Error loading the page:", err);
      throw err; // Re-throw the error so the test fails
    }
